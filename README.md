...
| Bash / WebFetch 用量 | ⚠️ **只提示，不拦截**（越过阈值时打印一行；硬限额没有） |
| 真正的根因判断 | ❌ 仍靠模型自己 |
| 重复会话 | ❌ 未解决 |
...
- **Bash / WebFetch 只提示不拦截** —— 越阈时打印一行，不影响执行。
  为什么不做硬拦：硬拦需要一个有数据支撑的合理阈值，当前没有实测分布；
  无数据时硬拦会误伤正常的深度调研任务
