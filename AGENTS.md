...
└─ tests/                          十三套测试（298 项，用 ls tests/*.py 枚举）
```
...
# 全部测试（298 项 = 12 套功能测试 298；检查器自身 19 项另计，提交前必须通过）
# ⚠️ 不要照抄这段清单 —— 用 glob 枚举，否则新增套件会被漏掉：
#     ls tests/*.py
python tests/four_gate_selftest.py        # 54
python tests/cmd_chain_test.py            # 21
python tests/adversarial_test.py          # 25
...
python tests/doc_consistency_test.py      # 19/19  ← 会递归跑上面全部，并核对文档数字
