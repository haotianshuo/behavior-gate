...
⚠️ 版本号纪律（2026-09-21 订正）：
   本文件内所有改动注释【统一标 VERSION 里那个版本号】。
   实测踩到：7 处注释误标 `3.5.16`，而 VERSION 是 `3.5.15` ——
   同一个文件里同一个改动被标成两个版本，将来无法回答
   「这个版本到底改了什么」。
   而 check_version_consistency 只查 VERSION/policy/文档，
   【不查代码注释】—— 所以这个不一致没有任何检查器能抓到。

   改 hook 内容时：先看 VERSION 是多少，注释就写多少。
   不要"预告"下一个版本号。
"""
...
#
# ⚠️ 3.5.15 B1 修法（2026-09-20 实测，外部核验 + 本机复现）：
#
#   缺陷：`派个子?代理` 里 `个` 是【必需字面量】—— 只有「派个子代理」命中，
#        而用户最自然的说法全部漏掉：
#            「派一个子代理」  ★ 不命中   ← 真实语料出现
#            「用三个子代理」  ★ 不命中   ← 真实语料出现（本会话实际发生）
#            「派 3 个子代理」 ★ 不命中
#            「多派几个子Agent」★ 不命中
#        后果：用户用自然中文授权 → agent_spawns 保持 0 → G1 deny，
#              门还反过来提示用户改用 `agent_spawns: N` 这种机器语法。
#        实测规模：72 条去重真实 prompt 中，提及子代理但解析出 permit 的仅 1 条。
#
#   修法要点（每条都对应一次实测失败，不是推测）：
#     ① 数量【必需】—— 曾写成可选 `(?:\d+|...)?`，退化成 `用+个+代理`。
#     ② 宾语【必需】—— 曾写成全可选尾部 `子?[Aa]?[Gg]?...`，可匹配空，
#        导致「用三个方法」「用三个小时」全部误命中（实测 47 条假阳性）。
#     ③ `(?<![遣委指调])` 左边界 —— 「派遣/委派/指派/调派」含「派」，
#        不加边界时「不自行派遣子Agent」被读成【授权】（实测复现）。
#     ④ 保留否定词分支 —— 与 SPAWN_FORBID 配套，否定形式在此被吃掉并排除。
#
# ⚠️ 3.5.15 P4 修法（2026-09-20，据《规则落地机制 V1.3》§12）：
#    动词表加 `让|叫|找|起|安排`，宾语表加 `监工`。
#    **必须与 SPAWN_FORBID 同步加** —— 否则新增动词的否定式
#    （「禁止让三个子代理」）会从 forbid 侧漏过、被 permit 侧接住。
#
# ⚠️⚠️ 3.5.15 二次修法（2026-09-20 23:48，外部核验发现回归）：
#    上一条修法【引入了新假阳性】—— 实测（旧版/新版对照）：
#        找三个代理商报价      旧 0 → 新 3   ★
#        找两个代理人谈合同     旧 0 → 新 3   ★
#        叫三个代理来开会      旧 0 → 新 3   ★
#        起三个代理进程       旧 0 → 新 3   ★
#        安排两个代理处理      旧 0 → 新 3   ★
#    根因：新动词（让/叫/找/起/安排）与【裸「代理」】组合，
#          在中文里大量是普通用法（代理商/代理人/代理进程/代理服务器）。
#          而「代理」在本项目技术语境里是高频词（ClaudeModelProxy 等）。
#    修法：【拆分动词组】——
#      · 原动词 派|用|调用|启用|开 → 保留完整宾语表（含裸「代理」）
#        （这是 3.5.15 之前的既有行为，本次不改变）
#      · 新动词 让|叫|找|起|安排   → 宾语【必须含「子」字】，
#        即 子Agent|子代理|监工|subagent，【不接受裸「代理」】
#    注：新动词的否定式仍由 SPAWN_FORBID 吃掉，危险方向实测 0/7 无回归。
#
# ⚠️ 依据订正：此前注释写「真实语料中…8/12 案例属此缺口」——
#    该依据【已被 V1.3 文档 §4.1.1 推翻】：提及子代理的 78 条 prompt
#    100% 是 promptSource='sdk'（脚本注入），真实交互输入里无样本。
#    故此处不再引用任何规模数字，只保留【可复现的形态】。
#
# ⚠️⚠️ 架构决定（2026-09-21，用户裁定 · 方案 B）——【停止补词】
#
#   用户原话：
#     「作为通用工具，每个人的习惯和口音不同，不能因为某个词就去激活，
#       这是不对的。至于语义理解，应该让大模型自己去理解，并自动纠正字词。
#       如果按你那样，激活词就无穷无尽了，也不可能随时更新。」
#
#   【本表就此冻结】。理由：
#     1. 枚举「什么样的话算授权」在结构上不可收敛 ——
#        口音/错别字/习惯用语/新说法是开放集合，每补一批就暴露下一批。
#        已补：派|用|调用|启用|开|让|叫|找|起|安排
#        仍漏：一些|多少|区间|无数量|自代理|子带理|…（无穷）
#     2. 本 hook 是 UserPromptSubmit —— 必须毫秒级返回，不能调模型。
#        故「语义判断交给模型」= 正则【只认无歧义的机器语法】，
#        自然语言的完整理解由【已在链路里的模型】承担。
#
#   【允许的改动】仅两类，除此之外不再扩表：
#     (a) 修【危险方向】—— 禁令被读成授权（如 P1 的 `并行调研`）
#         理由：安全方向，漏判禁令的代价高于少授权
#     (b) 修【已确证的真实缺陷】—— 有非 SDK 语料证据或可复现的误判
#
#   【不再做的改动】：
#     · 不为「某种自然说法没被识别」而补词（那是无穷集合）
#     · 不为 SDK 测试语料里出现的形态补词
#     · 不为错别字/口音补词（交给模型理解）
#
#   【逃生通道】（用户仍可怎么办）：
#     · 写 `agent_spawns: N` —— 机器语法，无歧义，始终有效
#     · 或直接用自然语言说，模型理解后自行决定是否派（本 hook 不拦）
#
#   已知边界（登记，不再修）：
#     「多拍几个自代理」「能派多少个就派多少个」「派上两个到三个子代理」
#     「多派一些子Agent」「三次交叉验证用子代理」
#     → 真实交互语料 68 条中【0 样本】，按 V1.5 §13.3 不为不存在场景加固。
SPAWN_PERMIT = re.compile(
    r"(可以派\s*[Aa]gent|(?<![不别禁勿])允许并行|"
    r"(?<![遣委指调])"
    # 否定式：与 SPAWN_FORBID 配套，动词表保持完整（两表必须对称）
    r"(?:(?:禁止|不许|不准|不要|别|不允许)\s*(?:派|用|调用|启用|开|让|叫|找|起|安排)|"
    # 原动词组：完整宾语表（含裸「代理」）—— 既有行为，不改变
    r"(?:多\s*)?(?:派|用|调用|启用|开))\s*"
    r"(?:\d+|[一二三四五六七八九十两]+|几)\s*个\s*"
    r"(?:子\s*[Aa]gent|子代理|代理|监工|subagents?)|"
    # 新动词组：宾语【必须含「子」字】，不接受裸「代理」
    r"(?:多\s*)?(?:让|叫|找|起|安排)\s*"
    r"(?:\d+|[一二三四五六七八九十两]+|几)\s*个\s*"
    r"(?:子\s*[Aa]gent|子代理|监工|subagents?)|"
    r"派个子?代理|多个子\s*[Aa]gent|并行调研|allow\s+subagents)",
    re.I)

# 用户口语里"不要派生"的信号
#
# ⚠️ 3.5.15 补充（B1 修法，2026-09-20 实测）：
#    原表只覆盖「不要派/别派/禁止派/不用 Agent/不要开后台/不要用子代理」，
#    不含「否定词 + 其他动词」。放宽 permit 表（见 SPAWN_PERMIT 注释）时，
#    「禁止调用两个子代理」会从 forbid 侧漏过、被 permit 侧接住 ——
#    **把用户的禁令读成授权**，正是本文件 :899-908 记载的那类事故。
#    实测复现：修前 `「禁止调用两个子代理」→ agent_spawns=3`。
#    故先补 forbid，再扩 permit。
#
# ⚠️ 3.5.15 P1+P4 修法（2026-09-20，据《规则落地机制 V1.3》§12）：
#    ① P1 危险方向缺口：permit 有 `并行调研`，forbid 无对应否定式 →
#       `禁止并行调研` / `不允许并行调研` 被判成【授权 3】。
#       与 :899-908 记载的事故同一形状，方向最危险。此处补上。
#    ② P4 动词表同步：permit 新增 `让|叫|找|起|安排` 后，
#       其否定式必须在这里【同步】被吃掉，否则漏到 permit 侧。
#       `监工` 作为宾语同步加入。
#
# ⚠️ 3.5.15 二次修法说明（2026-09-20 23:48）：
#    本表宾语【保留裸「代理」】，而 permit 的新动词组【不接受裸「代理」】
#    —— 这个不对称是【刻意的】，不是疏漏：
#      · forbid 侧宽松 = 安全方向（宁可多判禁令，不可漏判禁令）
#      · permit 侧严格 = 避免假阳性（「找三个代理商报价」不是派子代理）
#    两表【动词表】仍保持对称（让|叫|找|起|安排 两边都有），
#    只有【宾语表】在 permit 的新动词组上收紧。
SPAWN_FORBID = re.compile(
    r"(不要派|别派|禁止派|不用\s*[Aa]gent|不要开后台|别开后台|不要用子代理|"
    r"(?:禁止|不许|不准|不要|别|不允许)\s*"
    r"(?:派|用|调用|启用|开|让|叫|找|起|安排)\s*"
    r"(?:\d+|[一二三四五六七八九十两]+)?\s*个?\s*"
    r"(?:子\s*[Aa]gent|子代理|代理|监工|subagents?)|"
    r"(?:禁止|不许|不准|不要|别|不允许)\s*并行调研)",
    re.I)
...
#
# ⚠️ 3.5.15 B1 修法（2026-09-20）：后置疑问判据。
#    上面的 INTENT_QUESTION_CTX 只看信号【前面】26 字符，
#    而中文疑问常【后置】：
#        「用三个子代理行不行」      ← 疑问词在信号之后
#        「派三个子代理会不会太多」    ← 同上
#        「用三个子代理可以吗」      ← 同上
#    修前这 4 条只是"未解析"（巧合安全）；放宽 SPAWN_PERMIT 后
#    它们会变成【误授权】—— 实测不补这一层时 4 条全部 → 3(permit)。
#    故与 permit 修法【同时】补。
#
#    边界（诚实声明）：本判据看信号之后 15 字符，因此
#        「用三个子代理去做吧，可以吗」→ 被判为疑问（期望是授权）
#    这是【偏向少授权】的安全方向，且真实语料 0 次出现。
#
# ⚠️ 3.5.15 P4 补词（2026-09-20，据《规则落地机制 V1.3》§12）：
#    补 `合适` 与 `[么嘛]` —— 实测 `用三个子代理可以么` / `派几个子Agent合适`
#    当前误授权（base=3）。这两个是真实语料里的自然说法。
TRAILING_Q = re.compile(
    r"(行不行|可不可以|好不好|行吗|好吗|可以吗|会不会|是不是|要不要|该不该|"
    r"是否|怎么样|如何|多少|合适|吗[？?]?|呢[？?]?|[么嘛])")
...
def parse_budget_from_prompt(prompt, policy):
    ...
    if prompt:
        ...
        def _ok(m, rx):
            ...
            #
            # ⚠️ 3.5.15 B1 修法：再补【后置疑问】。
            #    中文疑问常【后置】——「用三个子代理行不行」「派三个子代理会不会太多」。
            #    前视窗口只看信号【前】26 字符，抓不到后置疑问词。
            #    修前这 4 条只是"未解析"（巧合安全）；放宽 permit 表后
            #    它们会变成【误授权】，所以必须同时补这一层。
            #    实测：不补时 4 条疑问句 → 3(permit)。
            # ⚠️ 3.5.15 P2 修法（2026-09-20，据《规则落地机制 V1.3》§4.2.2）：
            #    核心规则：疑问/条件/讨论词只有在【作用于派生信号本身】时
            #    才算「非断言」。判据 = 该词与信号之间没有其他实词（只有标点）。
            #
            #    依据（中文语法事实）：「验证 X 是否 Y」里的「是否」是
            #    【从句标句词】(complementizer)，不是疑问标记。
            #    把两者混为一谈抹掉授权 —— 这是本缺陷的语义根因。
            #
            #    实测（6 个根因，F3/F4 共谋）：
            #      · 删整句兜底(F4) → 只修好 7/29
            #      · 只收紧 TRAILING_Q(F3) → 只修好 7/29
            #      · 两者【同时】处理 → 29/29
            #    单改任何一个都会得到「没效果」的假结论。

            # ⚠️ 3.5.15 修法 D（2026-09-21）——两类语境用【不同】窗口扩展量。
            #
            #    修前：两类都用 +2。后果是 6 条禁令被判成 policy-default：
            #        不要派子代理 / 不要派太多子代理 / 不要派三个子代理
            #        不要用三个子代理 / 不要开后台 / 不要并行调研
            #
            #    根因：+2 让信号自身的前 2 个字进入 before 窗口。
            #      · 「要不要派…」（疑问）—— 需要 +2 才能抓到与信号
            #        共享首字的疑问词「要不要」。（这是 +2 原本的目的）
            #      · 「不要派…」（禁令）—— '不要' 也进了窗口，而
            #        DISCUSSION_CTX 含 '不要' → _gap 为空（无标点）
            #        → return False → 禁令被挡掉。
            #
            #    后果不只是少判一条禁令（n 仍为 0），而是【报错文案走错分支】：
            #      用户说「不要派子代理」→ 门回复「你从未在本会话里授权过」
            #      —— 伪造用户意图。与 :899-908 记载的事故同一形状。
            #
            #    修法：只有【疑问语境】需要 +2；讨论语境用 +0。
            #      这样「不要派」的 before 只剩信号之前的内容，不含 '不要'。
            #
            #    ⚠️ 不要「统一」成 +2 —— 那会让这 6 条禁令重新被挡。
            #       验证：tests/budget_safety_test.py 的「source 正确：明确禁令」
            #             与「『太多』是有限度，不是取消」两项。
            ext = 2 if rx is INTENT_QUESTION_CTX else 0
            before = said[max(0, m.start() - 26):m.start() + ext]
            _bm = rx.search(before)
            if _bm:
                # ② 语境词与信号之间出现小句边界 → 它管的是别的宾语，不抹掉
                #    例：「不要读别的文件，用三个子代理去验证」
                #        逗号隔开 → 「不要读别的文件」管的是"读"，不是"派"
                _gap = before[_bm.end():]
                if not re.search(r"[，。；！？、,;!?~]", _gap):
                    return False
            # ③ 后置疑问：只认【紧贴信号】或【位于整句末尾】的疑问词
            #    紧贴    → 问的是"派不派"        （用三个子代理行不行）
            #    末尾    → 尾疑问管整句          （…去做吧，可以吗）
            #    中段    → 从句标句词，是授权     （…是否真实）
            if rx is INTENT_QUESTION_CTX:
                _a = said[m.end():m.end() + 40]
                _q = TRAILING_Q.search(_a)
                if _q:
                    _mid = re.sub(r"[\s，。；！？、,;!?~]+", "", _a[:_q.start()])
                    _post = re.sub(r"[\s，。；！？、,;!?~]+", "", _a[_q.end():])
                    if not _mid or not _post:
                        return False
            # ④ 整句兜底（±40 字）已删除 —— 它抹掉了 22 条真实案例
            return True
...
def forbid_excerpt(prompt, maxlen=20):
    """取用户原话里【包含禁令命中】的片段，供 G1 报错文案引用。

    为什么需要它：
      budget_gate 的文案原先写死「你在 prompt 里说了不要派子代理」——
      无论用户实际说的是什么。用户说「不要开后台」，门却回
      「你说了不要派子代理」，那是【伪造用户原话】。

    为什么不能直接引用 SPAWN_FORBID 的命中文本：
      SPAWN_FORBID 的第一个分支是裸动词组（不要派|别派|…），
      它会【优先命中最短形式】：
          「不要派子代理」   → 命中 '不要派'（截断，读起来像半句话）
          「不要派太多子代理」→ 命中 '不要派'
      所以要从命中处【向两侧扩展到标点边界】。

    返回 (片段, 命中文本)；无禁令命中时返回 (None, None)。

    ⚠️ 边界：只取【第一个】通过的命中。一句话里有多处禁令时，
       文案只引用第一处 —— 那是"举例"，不是穷举。
    """
    if not prompt:
        return None, None
    ...
    def _ok(m, rx):
        # 与 parse_budget_from_prompt 内的 _ok 保持同一判据。
        # ⚠️ 两处必须同步 —— 若那边改了判据，这里也要改，
        #    否则文案会引用一个"没通过判定"的片段。
        ext = 2 if rx is INTENT_QUESTION_CTX else 0
        before = said[max(0, m.start() - 26):m.start() + ext]
        _bm = rx.search(before)
    ...
    PUNCT = "，。；！？、,;!?~\n"
    for m in SPAWN_FORBID.finditer(said):
        if not (_ok(m, DISCUSSION_CTX) and _ok(m, INTENT_QUESTION_CTX)):
            continue
        s = m.start()
        while s > 0 and said[s - 1] not in PUNCT and (m.start() - s) < maxlen:
            s -= 1
        e = m.end()
        while e < len(said) and said[e] not in PUNCT and (e - m.end()) < maxlen:
            e += 1
        frag = said[s:e].strip()
        return (frag or None), m.group(0)
    return None, None
